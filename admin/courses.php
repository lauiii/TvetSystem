...
// Fetch school years
$schoolYears = $pdo->query("SELECT id, year FROM school_years ORDER BY year DESC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if (!$program_id || !$school_year_id || empty($course_code) || empty($course_name)) {
        $error = 'Program, school year, course code and name are required.';
    } else {
        try {
            $stmt = $pdo->prepare('INSERT INTO courses (program_id, school_year_id, course_code, course_name, year_level, semester, instructor_id) VALUES (?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([$program_id, $school_year_id, $course_code, $course_name, $year_level, $semester, $instructor_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    ...
    if ($id <= 0 || !$program_id || !$school_year_id || empty($course_code) || empty($course_name)) {
        $error = 'All fields are required.';
    } else {
        try {
            $stmt = $pdo->prepare('UPDATE courses SET program_id = ?, school_year_id = ?, course_code = ?, course_name = ?, year_level = ?, semester = ?, instructor_id = ? WHERE id = ?');
            $stmt->execute([$program_id, $school_year_id, $course_code, $course_name, $year_level, $semester, $instructor_id, $id]);
...
<body>
                                    ...
                                    <?php foreach ($semesterCourses as $c): ?>
                                        <tr>
                                            ...
                                            <td style="display: flex; align-items: center; padding: 8px;">
                                                <button class="btn" onclick="editCourse(<?php echo $c['id']; ?>, '<?php echo htmlspecialchars($c['program_id']); ?>', '<?php echo htmlspecialchars($c['school_year_id']); ?>', '<?php echo htmlspecialchars($c['course_code']); ?>', '<?php echo htmlspecialchars($c['course_name']); ?>', '<?php echo htmlspecialchars($c['year_level']); ?>', '<?php echo htmlspecialchars($c['semester']); ?>', '<?php echo htmlspecialchars($c['instructor_id'] ?? ''); ?>')">Edit</button>
    ...
    <div id="editModal" class="modal" style="display:none;">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label>School Year</label>
                    <select name="school_year_id" id="editSchoolYearId" required>
                        <option value="">-- choose school year --</option>
                        <?php foreach ($schoolYears as $sy): ?>
                            <option value="<?php echo $sy['id']; ?>"><?php echo htmlspecialchars($sy['year']); ?></option>
                        <?php endforeach; ?>
    ...
    <script>
        function editCourse(id, programId, schoolYearId, courseCode, courseName, yearLevel, semester, instructorId) {
